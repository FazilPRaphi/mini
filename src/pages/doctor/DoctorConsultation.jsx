...
const DoctorConsultation = () => {
  ...
  const loadHistory = async (patientId, bookingData) => {
    ...
    // Fetch reports
    const { data: reportsData } = await supabase.from("patient_reports").select("*").eq("patient_id", patientId).order("created_at", { ascending: false });
    setReports(reportsData || []);

    // Fetch the latest profile data explicitly to ensure medical_history is up to date
    const { data: profileData } = await supabase.from("profiles").select("full_name, age, gender, phone, medical_history").eq("id", patientId).single();
    
    // Update the selected item with the fresh profile data
  ...
  const handleSelect = (b) => { loadHistory(b.patient_id, b); };
            ...
            <div style={{ ...card, border: "1.5px solid #BEE3F8" }}>
              ...
              {reports.length > 0 && (
                <div style={{ marginTop: 20 }}>
                  <h3 style={{ fontSize: 13, fontWeight: 700, color: "#1A202C", marginBottom: 10 }}>Lab Reports & Docs</h3>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {reports.map((r) => (
                      <a 
                        key={r.id} 
                        ...
                        onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#0BC5EA"; e.currentTarget.style.boxShadow = "0 2px 8px rgba(11, 197, 234, 0.15)"; }}
                      ...
                      >
                        <div style={{
                          ...
                          <span style={{ fontSize: 18, margin: 'auto' }}>{r.file_type.startsWith('image/') ? '🖼️' : '📄'}</span>
                        </div>
                        <div style={{ minWidth: 0, flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
                          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: "#2D3748", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{r.file_name}</p>
                          <p style={{ margin: 0, fontSize: 11, color: "#718096" }}>Added {new Date(r.created_at).toLocaleDateString()}</p>
                        </div>
                        <span style={{ color: "#0BC5EA", fontSize: 16 }}>↗</span>
                      </a>
                    ))}
                  </div>
                </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ ...card }}>
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              {/* Header row */}
              {meds.length > 0 && (
                <div className="hidden sm:grid sm:grid-cols-4 gap-2 mb-2 px-1">
          ...
          <div style={{ ...card }}>
            ...
            {selected && (
              <div style={{ padding: 16, background: "#FFF5F5", borderRadius: 12, border: "1px solid #FED7D7", marginBottom: 16 }}>
                <h3 style={{ fontSize: 13, fontWeight: 700, color: "#C53030", margin: "0 0 8px", display: "flex", alignItems: "center", gap: 6 }}>
                  <span>⚠️</span> Patient Reported History
                </h3>
                <p style={{ fontSize: 13, color: "#742A2A", margin: 0, lineHeight: 1.5 }}>
                  {selected.profiles?.medical_history || "No personal history provided by the patient."}
                </p>
              </div>
            )}

            {history.length === 0 && (
              <p style={{ color: "#A0AEC0", fontSize: 13 }}>{selected ? "No previous clinic visits recorded." : "Select a patient to view history"}</p>
            )}
